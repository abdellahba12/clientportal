...
export default function UpgradeCancel() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', flexDirection: 'column', gap: 16, textAlign: 'center', padding: 24 }}>
      ...
      <button onClick={() => navigate('/upgrade')} className="btn btn-primary" style={{ width: 'auto' }}>Intentar de nuevo</button>
      <button onClick={() => navigate('/')} className="btn btn-secondary" style={{ width: 'auto' }}>Volver al inicio</button>
    </div>
  );
}
